...
from utils.helpers import next_proxy

logger = getLogger(__name__)

def handle_exception(client, error) -> None:
...
class InstagramClient:
    ...
    def get_client(
        ...
        client.handle_exception = handle_exception # type: ignore
        user_instagram_session_is_exist = os.path.exists(user_instagram_session)
        dump_path = Path(f"{login_directory}/{self.username}_{telegram_user_id}.json")
        if user_instagram_session_is_exist:
            client.load_settings(Path(user_instagram_session))
            ...
            if save_session:
                ...
                client.dump_settings(dump_path)
            client.login(
                username=self.username,
                password=self.password,
                verification_code=self.verification_code,
